...
class USAM_List_Table_contact_report extends USAM_Main_Report_List_Table
{		
	...
	{		
		...
		$records = array();	
		$contact_id = (int)$this->get_filter_value( 'contact' );
		if ( !empty($contact_id) )
		{			
			...
			if ( !empty($selected) )
				$date_query['dayofweek'] =  array_map('intval', (array)$selected);			
			...
			$query = array(
				...
				'links_query' => [['object_type' => 'contact', 'object_id' => $contact_id]],
				'order' => 'DESC', 					
			);				
