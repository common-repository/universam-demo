...
class USAM_List_Table_trigger_email_newsletters extends USAM_Table_Newsletter
{	
    ...
    {		
		...
		if( $item->status == 5 || $item->status == 4 || $item->status == 6  )
		{
			...
			$actions = array('view' => '<a class="usam-view-link" href="'.add_query_arg( array('form_name' => 'email_newsletter', 'form' => 'view', 'id' => $item->id), $this->url).'">'.__('Отчет', 'usam').'</a>') + $actions;
