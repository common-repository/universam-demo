...
class USAM_List_Table_commissions extends USAM_List_Table 
{	
	...
	function column_status( $item )
    {
		...
		echo "<span class='$class item_status'>".usam_get_marketplace_commission_status_name( $item->status )."</span>";
    }	
	
	function column_seller( $item ) 
    {	
		$seller = usam_get_seller_data( $item->seller_id ); 
		echo "<a href='".admin_url("admin.php?page=marketplace&tab=sellers&form=view&form_name=seller&id=".$seller['id'])."'>".$seller['name']."</a>";
    }
	...
	function get_columns()
	{
		...
		if ( get_option('usam_options_earning_marketplace', 'sales_commission_seller' ) == 'sales_commission_seller' )
			$columns['order_id'] = __('Заказ', 'usam');
