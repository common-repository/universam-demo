...
class USAM_List_Table_sms_newsletter_templates extends USAM_Table_Newsletter
{
    ...
    {		
		...
		if( $item->status == 5 || $item->status == 4 || $item->status == 6  )
		{
			...
			$actions = ['view' => '<a class="usam-view-link" href="'.add_query_arg(['form_name' => 'sms_newsletter', 'form' => 'view', 'id' => $item->id], $this->url).'">'.__('Отчет', 'usam').'</a>'] + $actions;	
		}
		else
