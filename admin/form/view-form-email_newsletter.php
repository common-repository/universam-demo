...
class USAM_Form_email_newsletter extends USAM_View_Form
{	
	protected function get_title_tab()
	{ 	
		return sprintf( __('Рассылка &laquo;%s&raquo;','usam'), $this->data['subject'] );
	}
	...
	{		
		...
		$this->data['email'] = $mailbox['email'];
		$this->tabs = array( 
			['slug' => 'newsletter_contact', 'title' => __('Контакты','usam')], 			 	
			['slug' => 'newsletter_company', 'title' => __('Компании','usam')], 			
			['slug' => 'letter', 'title' => __('Письмо','usam')], 				
		);		
		if ( !empty($this->data) )
		{
		...
		$this->js_args['sending_statuses'] = usam_get_newsletter_sending_statuses();		
	}	
	...
	{	
		...
		<div class="view_data">
			<div class ="view_data__row view_data__highlighted">
				<div class ="view_data__name"><?php _e('Почта', 'usam'); ?>:</div>
				<div class ="view_data__option"><?php echo $this->data['email']; ?></div>
			</div>
			<div class ="view_data__row view_data__highlighted">
	...
	function display_tab_letter()
	{
		echo htmlspecialchars_decode( usam_get_newsletter_metadata( $this->data['id'], 'body' ) );
	}	
}
?>