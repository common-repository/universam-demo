...
class USAM_Form_viber extends USAM_Form_Social_Network
{					
	protected function get_title_tab()
	{ 	
		if ( $this->id != null )
			$title = sprintf( __('Изменить Viber &#171;%s&#187;','usam'), $this->data['name'] );
		else
			$title = __('Добавить профиль Viber', 'usam');	
		return $title;
	}
			...
			<div class ="edit_form__item">
				...
				<div class ="edit_form__item_option">
					<select name="from_group" id='from_group'>							
						<option value='1' <?php selected(1, $this->data['from_group']); ?>><?php esc_html_e( 'имя паблик аккаунта', 'usam'); ?></option>		
