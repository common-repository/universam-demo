...
$columns = [ 
	...
	['id' => 'source', 'name' => __('Источник', 'usam'), '' => ''],	
...
?>
<list-table query="contacts" :args="argsContact" :columns='<?php echo json_encode( $columns ); ?>'>
	<template v-slot:tbody="slotProps">
		<tr v-for="(item, k) in slotProps.items">
			<td class="column-name">						
				<a :href="'<?php echo add_query_arg(['form' => 'view', 'form_name' => 'contact']); ?>&id='+item.id">{{item.appeal}}</a>
			</td>
			<td class="column-source">{{item.source}}</td>			
