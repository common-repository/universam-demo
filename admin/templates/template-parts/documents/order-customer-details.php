<div class = 'edit_form'>	
	...
	<div class ="edit_form__item" v-if="addresses.length">
		<div class ="edit_form__item_name"><?php esc_html_e('Адрес', 'usam'); ?>:</div>
		<div class ="edit_form__item_option">
			<span v-if="!edit_data && address.id==data.address_id" v-for="address in addresses" v-html="address.name"></span>
			<select v-model="data.address_id" v-if="edit_data" name="address_id">			
				<option v-for="address in addresses" :value="address.id" v-html="address.name"></option>
			</select>		
		</div>
	</div>
	<div class ="edit_form__item">
		<div class ="edit_form__item_name"><label><?php esc_html_e( 'Тип плательщика', 'usam'); ?>:</label></div>
		<div class ="edit_form__item_option">
...
<div class ="edit_form" v-for="group in propertyGroups" v-if="check_group(group.code) && group.type_payers.includes(data.type_payer)">
	...
	<div class ="edit_form__item" v-for="(property, k) in properties" v-if="property.group==group.code" :class="{'edit_form__row_error':property.error}">
