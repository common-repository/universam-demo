<?php
require_once( USAM_FILE_PATH . '/admin/grid-view/orders_grid_view.class.php' );
class USAM_Leads_Grid_View extends USAM_Orders_Grid_View
{		
	protected function counter_panel() 
	{ 
		printf( __('Всего: %s на сумму %s', 'usam'), '<span class="counter_panel__total_items">{{items.length}}</span>', '<span class="counter_panel__total_price" v-html="formatted_sum"></span>' );
	}
	...
	{ 		
		...
		?>								
		<div v-if="(column.visibility || column.number > 0) && !column.close" class="grid_column" v-for="(column, k) in columns">
				...
				<div class="grid_view__item" v-if="item.status==column.internalname" v-for="(item, i) in items" draggable='true' @dragstart="drag($event, i, k)" @dragend="dragEnd($event, i)" :class="{'grid_view__item_checked':item.checked}">
					<div class="grid_view__item_wrapper" @click="checked(i,$event)">
						<div class="grid_item__row grid_item__title">
							<a :href="'<?php echo admin_url("admin.php?page=orders&tab=leads&form=view&form_name=lead&id=") ?>'+item.id" :class="{'item_status_valid item_status':item.paid}" v-html="item.name"></a>
							<a v-if="!item.checked" :href="'<?php echo admin_url("admin.php?page=orders&tab=leads&form=view&form_name=lead&id=") ?>'+item.id" class="document_totalprice" v-html="item.totalprice_currency"></a>
							<div v-else class="grid_view_checkbox"></div>
						</div>
						<div class="grid_item__row grid_item__row_customer" v-if="item.properties.billingfirstname !== undefined">
							<a v-if="item.contact_id && item.contact.firstname==item.properties.billingfirstname.value && item.contact.firstname==item.properties.billingfirstname.value" :href="'<?php echo admin_url("admin.php?page=orders&tab=orders&form=view&form_name=contact&id=") ?>'+item.contact_id">
						...
						<div class="grid_item__row grid_item__row_customer" v-if="item.properties.company !== undefined">	
							<a v-if="item.company_id" :href="'<?php echo admin_url("admin.php?page=orders&tab=leads&form=view&form_name=company&id=") ?>'+item.company_id" v-html="item.properties.company.value"></a>
						</div>						
						<div class="grid_item__row order_date">
							<span>{{localDate(item.date_insert,'<?php echo get_option('date_format', 'Y/m/j') ?>')}}</span>
						</div>
		...
		<teleport to="body">			
			...
			if( $possibility_to_call )
			{
				?><phone-call :phone="phone" :object_id="object_id" :object_type="'lead'"></phone-call><?php		
			}
			?>
		</teleport>
		<?php 	
