<?php
// Класс работы с визитами
class USAM_Visits_Query 
{
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'contact_id' => 'd', 'location_id' => 'd', 'source' => 's', 'ip' => 'd', 'views' => 'd', 'visits' => 'd', 'date_update' => 's', 'date_insert' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_visits', $this );
		...
		{
			if ( $field == 'all' )
				$this->fields[] = USAM_TABLE_VISITS.".*";		
			elseif ( $field == 'count' )
				$this->fields[] = "COUNT(*) AS count";	
			elseif ( $field == 'meta_value' )
				$this->fields[] = USAM_TABLE_VISIT_META.".meta_value AS meta_value";	
			elseif ( $field == 'device' )
			{
				$this->fields[] = "device.meta_value AS device";					
				$device_join = true;
			}
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_VISITS.".$field";
		}
		if ( !count($this->fields) )
			$this->query_fields = USAM_TABLE_VISITS.".*";
		else
		...
		$this->query_from = "FROM ".USAM_TABLE_VISITS;
		$this->query_where = "WHERE 1=1";	
		$page_viewed_join = false;		
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( [$date_parameters], USAM_TABLE_VISITS );
			$this->query_where .= $date_query->get_sql();
		}
		if ( !empty($qv['date_query']) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_VISITS );
		...
		if ( !empty($contact_id) ) 
			$this->query_where .= " AND ".USAM_TABLE_VISITS.".contact_id IN (".implode( ",",  $contact_id ).")";
		if ( isset($qv['contact_id__not_in']) )
			$this->query_where .= " AND ".USAM_TABLE_VISITS.".contact_id NOT IN (".implode( ",",  (array) $qv['contact_id__not_in'] ).")";
		...
		if ( !empty( $source ) ) 
		{
			$this->query_where .= " AND ".USAM_TABLE_VISITS.".source IN ('".implode( "','",  $source )."')";		
		}		
		if ( !empty($source__not_in) ) 
		{			
			$this->query_where .= " AND ".USAM_TABLE_VISITS.".source NOT IN ('".implode( "','",  $source__not_in )."')";
		}	
		...
		if ( $qv['groupby'] != '' )
		{
			...
			{				
				switch ( $_value ) 
				{
					...
					break;					
					case 'meta_key' :
						$ordersby_array[] = USAM_TABLE_VISIT_META.".meta_value";					
					break;	
					case 'device' :
						$ordersby_array[] = "device.meta_value";					
					break;						
					default:
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_VISITS.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			if ( $qv['search_columns'] )			
				$search_columns = array_intersect( $qv['search_columns'], ['id', 'contact_id', 'source']);
			if ( ! $search_columns ) 
			{
				if ( is_numeric($search) )
					$search_columns = ['id', 'contact_id'];				
				else
					$search_columns = ['source'];
			}	
			$search_columns = apply_filters( 'usam_visits_search_columns', $search_columns, $search, $this );
		...
		{	
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_VISITS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_VISITS.".id NOT IN ($ids)";
		}		
		if ( !empty( $qv['ip'] ) ) 
		{		
			$ip = implode( "','",  (array)$qv['ip'] );		
			$this->query_where .= " AND ".USAM_TABLE_VISITS.".ip IN ('$ip')";
		}		
		if ( isset($qv['online']) ) 
		{			
			if ( $qv['online'] ) 
				$this->query_where .= " AND ".USAM_TABLE_VISITS.".date_update >='".date( 'Y-m-d H:i:s', USAM_CONTACT_ONLINE)."'";
			else
				$this->query_where .= " AND ".USAM_TABLE_VISITS.".date_update <= '".date( 'Y-m-d H:i:s', USAM_CONTACT_ONLINE)."'";
		}
		if ( isset($qv['term_ids']) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['term_ids'] ) );
			$this->query_where .= " AND ".USAM_TABLE_PAGE_VIEWED.".term_id IN ($ids)";
			$page_viewed_join = true;
		}
		$this->meta_query = new USAM_Meta_Query();
		$this->meta_query->parse_query_vars( $qv );	
		if ( !empty( $this->meta_query->queries ) ) 
		{
			$clauses = $this->meta_query->get_sql( 'visit', USAM_TABLE_VISIT_META, USAM_TABLE_VISITS, 'id', $this );
		...
		if( $page_viewed_join )
		{
			$this->query_join .= "INNER JOIN ".USAM_TABLE_PAGE_VIEWED." ON (".USAM_TABLE_PAGE_VIEWED.".visit_id=".USAM_TABLE_VISITS.".id)";	
			if ( strripos($this->query_fields, 'DISTINCT')  === false ) 
				$this->query_fields = 'DISTINCT ' . $this->query_fields;
		}	
		if( $device_join )
		{
			$this->query_join .= "INNER JOIN ".USAM_TABLE_VISIT_META." AS device ON (device.visit_id=".USAM_TABLE_VISITS.".id AND device.meta_key='device')";	
		}	
		$conditions_query = new USAM_Conditions_Query();
		$this->query_where .= $conditions_query->get_sql_clauses( $qv );
		if ( $this->distinct ) 
			$this->query_fields = 'DISTINCT ' . $this->query_fields;
		do_action_ref_array( 'usam_pre_visits_query', array( &$this ) );
	}
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_visit' );	
		return $data;
	}
	...
	public function query()
	{
		...
		if ( !$this->results )
			return;	
		if ( isset($qv['count_total'] ) && $qv['count_total'] )
			$this->total = (int)$wpdb->get_var( apply_filters( 'found_visits_query', 'SELECT FOUND_ROWS()' ) );				
		...
		{ 
			...
			foreach ( $this->results as $result ) 
				$ids[] = $result->id; 
			if ( $qv['cache_meta'] )	
				usam_update_cache( $ids, [USAM_TABLE_VISIT_META => 'visit_meta'], 'visit_id' );
		}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		foreach ( $cols as $col ) 
		{
			if ( 'id' == $col ) 
			{
				$searches[] = $wpdb->prepare( USAM_TABLE_VISITS.".$col = %s", $string );
			} 
			else 
			{
				$searches[] = $wpdb->prepare( USAM_TABLE_VISITS.".$col LIKE %s", $like );
			}
		}
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_VISITS.'.'.$orderby;
		elseif ( $orderby == 'date' )
			$_orderby = USAM_TABLE_VISITS.'.date_insert';		
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) )
		{
			$include = wp_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".USAM_TABLE_VISITS.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_visits( $args = [] )
{
	$args['count_total'] = false;
	$class = new USAM_Visits_Query( $args );	
