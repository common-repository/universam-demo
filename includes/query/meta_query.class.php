<?php
//  Класс работы с дополнительными полями 
class USAM_Meta_Query 
{
	...
	public function get_sql_for_clause( &$clause, $parent_query, $clause_key = '' ) {
		...
		if ( false === $alias ) {
			...
			$alias = $i ? $this->meta_table . $i : $this->meta_table;
		...
		if ( array_key_exists( 'key', $clause ) ) {
			if ( 'NOT EXISTS' === $meta_compare ) {
				...
				if ( in_array( $meta_compare_key, array( '!=', 'NOT IN', 'NOT LIKE', 'NOT EXISTS', 'NOT REGEXP' ), true ) ) 
				{
					...
					$i                     = count( $this->table_aliases );
					$subquery_alias        = $i ? $this->meta_table . $i : $this->meta_table;
					$this->table_aliases[] = $subquery_alias;

					$meta_compare_string_start  = 'NOT EXISTS (';
					$meta_compare_string_start .= "SELECT 1 FROM $wpdb->postmeta $subquery_alias WHERE $subquery_alias.post_ID = $alias.post_ID ";
	...
	protected function find_compatible_table_alias( $clause, $parent_query ) {
		...
		foreach ( $parent_query as $sibling ) 
		{
			...
			if ( 'OR' === $parent_query['relation'] ) {
				$compatible_compares = array( '=', 'IN', 'BETWEEN', 'LIKE', 'BEGINS', 'ENDS', 'REGEXP', 'RLIKE', '>', '>=', '<', '<=' );
