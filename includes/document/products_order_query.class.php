<?php
//Товары заказа
class USAM_Products_Order_Query 
{
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_products_order', $this );
		...
		$order_join = false;
		if ( 'all' == $qv['fields'] ) 		
			$this->query_fields = USAM_TABLE_PRODUCTS_ORDER.".*";
		else
		{
			...
			{	
				...
				else
				{
					...
					$this->query_fields[] = USAM_TABLE_PRODUCTS_ORDER.".$field";
				}
			}
		...
		$this->query_from = "FROM ".USAM_TABLE_PRODUCTS_ORDER;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( array( $date_parameters ), USAM_TABLE_PRODUCTS_ORDER );
			$this->query_where .= $date_query->get_sql();
		}	
		if ( !empty( $qv['date_query'] ) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_PRODUCTS_ORDER );
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_PRODUCTS_ORDER.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			$search_columns = apply_filters( 'usam_products_order_search_columns', $search_columns, $search, $this );
		...
		{	// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_PRODUCTS_ORDER.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_PRODUCTS_ORDER.".id NOT IN ($ids)";
		}						
		if ( isset($qv['order_id']) ) 
		{
			$order_id = implode( ',',  (array)$qv['order_id'] );
			$this->query_where .= " AND ".USAM_TABLE_PRODUCTS_ORDER.".order_id IN ($order_id)";
		}						
		if ( !empty($qv['order_status']) ) 
		{
			if ( is_array($qv['order_status']) ) 
				$order_status = $qv['order_status'];			
			elseif ( is_string($qv['order_status']) && !empty($qv['order_status']) )
			{
				if ( $qv['order_status'] != 'all' )
					$order_status = array_map( 'trim', explode( ',', $qv['order_status'] ) );
			}
			elseif ( is_numeric($qv['order_status']) )
				$order_status = array($qv['order_status']);
				
			if ( !empty($order_status) ) 
			{
				$this->query_where .= " AND ".USAM_TABLE_ORDERS.".status IN ('".implode("','",  $order_status)."')";	
				$order_join = true;
			}
		}	
		if ( isset($qv['order_status__not_in']) ) 
		{
			$order_status__not_in = (array) $qv['order_status__not_in'];
			$this->query_where .= " AND ".USAM_TABLE_ORDERS.".status NOT IN ('".implode("','",  $order_status__not_in)."')";
			$order_join = true;
		}
		...
		if ( isset($qv['order_manager']) ) 
		{
			$order_manager = implode( "','",  (array)$qv['order_manager'] );
			$this->query_where .= " AND ".USAM_TABLE_ORDERS.".manager IN ('$order_manager')";
			$order_join = true;
		}	
		if ( isset($qv['order_paid']) ) 
		{
			$paid = implode( "','",  (array)$qv['order_paid'] );
			$this->query_where .= " AND ".USAM_TABLE_ORDERS.".paid IN ('$paid')";
			$order_join = true;
		}	
		if ( isset($qv['order_type_price']) ) 
		{
			$type_price = implode( "','",  (array)$qv['order_type_price'] );
			$this->query_where .= " AND ".USAM_TABLE_ORDERS.".type_price IN ('$type_price')";
			$order_join = true;
		}		
		if ( isset($qv['order_payer']) ) 
		{
			$payer = implode( "','",  (array)$qv['order_payer'] );
			$this->query_where .= " AND ".USAM_TABLE_ORDERS.".payer IN ('$payer')";
			$order_join = true;
		}		
		if ( isset($qv['companies']) ) 
		{
		...
		if ( isset($qv['contacts']) ) 
		{
			...
			$this->query_where .= " AND ".USAM_TABLE_ORDERS.".contact_id IN ('$contacts')";
			$order_join = true;
		}	
		if ( isset($qv['product_ids']) ) 
		{
			$product_ids = implode( "','",  (array)$qv['product_ids'] );
			$this->query_where .= " AND ".USAM_TABLE_PRODUCTS_ORDER.".product_id IN ('$product_ids')";
		}		
		$conditions_query = new USAM_Conditions_Query();
		$this->query_where .= $conditions_query->get_sql_clauses( $qv );
		if ( $order_join )
			$this->query_join .= " INNER JOIN ".USAM_TABLE_ORDERS." ON (".USAM_TABLE_ORDERS.".id = ".USAM_TABLE_PRODUCTS_ORDER.".order_id)";		
		do_action_ref_array( 'usam_pre_products_order_query', array( &$this ) );
	}
	
	public function handling_data_types( $data )
	{
		foreach (['id', 'product_id', 'order_id', 'product_day', 'bonus', 'used_bonuses'] as $column ) 
		{			
	...
	public function query()
	{
		...
		if ( isset($qv['count_total'] ) && $qv['count_total'] )
			$this->total = (int)$wpdb->get_var( apply_filters( 'found_products_order_query', 'SELECT FOUND_ROWS()' ) );
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		{
			$_orderby = USAM_TABLE_PRODUCTS_ORDER.'.'.$orderby;
		} 
		elseif ( 'sum' == $orderby ) 
		{
			$_orderby = 'sum';
		...
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) )
		{
			...
			$_orderby = "FIELD( ".USAM_TABLE_PRODUCTS_ORDER.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_products_order_query( $args )
{
	$args['count_total'] = false;	
	$products_order = new USAM_Products_Order_Query( $args );	
	$results = $products_order->get_results();	
