...
class USAM_Order_Product_Taxes_Query
{
	...
	public function prepare_query( $query = array() ) 
	{
		...
		$order_join = false;
		if ( 'all' == $qv['fields'] ) 		
			$this->query_fields = USAM_TABLE_TAX_PRODUCT_ORDER.".*";
		else
		{
			...
			{	
				...
				else
				{
					...
					$this->query_fields[] = USAM_TABLE_TAX_PRODUCT_ORDER.".$field";
				}
			}
		...
		$this->query_from = "FROM ".USAM_TABLE_TAX_PRODUCT_ORDER;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( array( $date_parameters ), USAM_TABLE_TAX_PRODUCT_ORDER );
			$this->query_where .= $date_query->get_sql();
		}	
		if ( !empty( $qv['date_query'] ) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_TAX_PRODUCT_ORDER );
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_TAX_PRODUCT_ORDER.".id $order";
		}
		...
		{	// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_TAX_PRODUCT_ORDER.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_TAX_PRODUCT_ORDER.".id NOT IN ($ids)";
		}						
		if ( isset($qv['order_id']) ) 
		{
			$order_id = implode( ',',  (array)$qv['order_id'] );
			$this->query_where .= " AND ".USAM_TABLE_TAX_PRODUCT_ORDER.".order_id IN ($order_id)";
		}
		if ( isset($qv['is_in_price']) ) 
		{
			$is_in_price = $qv['is_in_price']?1:0;
			$this->query_where .= " AND ".USAM_TABLE_TAX_PRODUCT_ORDER.".is_in_price=$is_in_price";
		}		
		...
		if ( isset($qv['product_ids']) ) 
		{
			...
			$this->query_where .= " AND ".USAM_TABLE_TAX_PRODUCT_ORDER.".product_id IN ('$product_ids')";
		}		
		$conditions_query = new USAM_Conditions_Query();
		$this->query_where .= $conditions_query->get_sql_clauses( $qv );
		if ( $order_join )
			$this->query_join .= " INNER JOIN ".USAM_TABLE_ORDERS." ON (".USAM_TABLE_ORDERS.".id = ".USAM_TABLE_TAX_PRODUCT_ORDER.".order_id)";		
		do_action_ref_array( 'usam_pre_order_product_taxes_query', array( &$this ) );
	}
	
	public function handling_data_types( $data )
	{
		foreach (['id', 'product_id', 'order_id', 'tax_id', 'is_in_price', 'rate'] as $column ) 
		{			
	...
	public function query()
	{
		...
		if ( $qv['products_cache'] )
		{
			...
			{
				usam_get_products( array( 'post__in' => $product_ids, 'update_post_term_cache' => true ), true );
			}
		}
	}
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		{
			$_orderby = USAM_TABLE_TAX_PRODUCT_ORDER.'.'.$orderby;
		} 
		elseif ( 'sum' == $orderby ) 
		{
			$_orderby = 'sum';
		...
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) )
		{
			...
			$_orderby = "FIELD( ".USAM_TABLE_TAX_PRODUCT_ORDER.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_order_product_taxes_query( $args )
{
	$args['count_total'] = false;	
	$query = new USAM_Order_Product_Taxes_Query( $args );	
