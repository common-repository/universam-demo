<?php
// Класс работы с подписками
class USAM_Subscriptions_Query 
{
	public $query_vars = [];
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'type_price' => 's', 'totalprice' => 'f', 'customer_id' => 'd', 'customer_type' => 's', 'manager_id' => 'd', 'status' => 's', 'date_insert' => 's', 'start_date' => 's', 'end_date' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_subscriptions', $this );
		...
		{		
			if ( $field == 'all' )
				$this->fields[] = USAM_TABLE_SUBSCRIPTIONS.".*";	
			elseif ( $field == 'count' )
				$this->fields[] = "COUNT(*) AS count";	
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_SUBSCRIPTIONS.".$field";
		}
		if ( !count($this->fields) )
			$this->query_fields = USAM_TABLE_SUBSCRIPTIONS.".*";
		else
		...
		$this->query_from = "FROM ".USAM_TABLE_SUBSCRIPTIONS;
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( [$date_parameters], USAM_TABLE_SUBSCRIPTIONS );
			$this->query_where .= $date_query->get_sql();
		}
		if ( !empty($qv['date_query']) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_SUBSCRIPTIONS );
		...
		{
			$orderby_array[] = USAM_TABLE_SUBSCRIPTIONS.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			if ( $qv['search_columns'] )
				$search_columns = array_intersect( $qv['search_columns'], array( 'id', 'customer', 'product', 'product_id' ) );
			if ( ! $search_columns ) 
			{
				if ( is_numeric($search) )
					$search_columns = array('id', 'product_id' );			
				else
					$search_columns = array('id', 'customer', 'product');
			}	
			$search_columns = apply_filters( 'usam_subscriptions_search_columns', $search_columns, $search, $this );
		...
		{
			$ids = implode( ',', $include );			
			$this->query_where .= " AND ".USAM_TABLE_SUBSCRIPTIONS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_SUBSCRIPTIONS.".id NOT IN ($ids)";
		}	
		if ( !empty($qv['status']) ) 
		{
			...
			if ( !empty($status) ) 
				$this->query_where .= " AND ".USAM_TABLE_SUBSCRIPTIONS.".status IN ('".implode("','",  $status)."')";	
		}	
		if ( isset($qv['status__not_in']) ) 
		{
			$status__not_in = (array) $qv['status__not_in'];
			$this->query_where .= " AND ".USAM_TABLE_SUBSCRIPTIONS.".status NOT IN ('".implode("','",  $status__not_in)."')";
		}
		if ( !empty( $qv['customer_type'] ) )
		{		
			$customer_type = (array)$qv['customer_type'];
			$this->query_where .= " AND ".USAM_TABLE_SUBSCRIPTIONS.".customer_type IN ('".implode( "','", $customer_type )."')";
		}	
		if ( !empty( $qv['customer_id'] ) )
		{		
			$customer_id = (array)$qv['customer_id'];
			$this->query_where .= " AND ".USAM_TABLE_SUBSCRIPTIONS.".customer_id IN (".implode( ",", $customer_id ).")";
		}		
		if ( !empty( $qv['product_id'] ) )
		{		
			$product_ids = (array)$qv['product_id'];
			$this->query_where .= " AND ".USAM_TABLE_SUBSCRIPTIONS.".product_id IN (".implode( ",", $product_ids ).")";
		}			
		$this->meta_query = new USAM_Meta_Query();
		$this->meta_query->parse_query_vars( $qv );	
		if ( !empty( $this->meta_query->queries ) ) 
		{
			$clauses = $this->meta_query->get_sql( 'subscription', USAM_TABLE_SUBSCRIPTION_META, USAM_TABLE_SUBSCRIPTIONS, 'id', $this );
		...
		do_action_ref_array( 'usam_pre_subscriptions_query', array( &$this ) );
	}	
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_subscription' );
		return $data;
	}
	...
	public function query()
	{
		...
		if ( !empty($this->results) && ( $qv['cache_meta'] || $qv['cache_products'] ) )
		{			
			$ids = array();	
			...
			if ( !empty($ids) )
			{
				...
				if ( $qv['cache_meta'] )
				{				
					$tables[USAM_TABLE_SUBSCRIPTION_META] = 'subscription_meta';
				}	
				if ( $qv['cache_products'] )
				{				
					$tables[USAM_TABLE_SUBSCRIPTION_PRODUCTS] = 'subscription_products';
				}				
				if ( !empty($tables) )
					usam_update_cache( $ids, $tables, 'subscription_id' );	
			}
		}
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		foreach ( $cols as $col ) 
		{
			if ( 'id' == $col ) 
			{
				$searches[] = $wpdb->prepare( USAM_TABLE_SUBSCRIPTIONS.".$col = %s", $string );
			} 
			elseif ( 'customer' == $col ) 
			{
				//$searches[] = $wpdb->prepare( USAM_TABLE_SUBSCRIPTIONS.".$col = %s", $string );
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_SUBSCRIPTIONS.'.'.$orderby;
		elseif ( $orderby == 'date' )
			$_orderby = USAM_TABLE_SUBSCRIPTIONS.'.date_insert';
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) ) {
			$include = wp_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".USAM_TABLE_SUBSCRIPTIONS.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_subscriptions( $query = array() )
{	
	$query['count_total'] = false;
	$subscriptions = new USAM_Subscriptions_Query( $query );	
