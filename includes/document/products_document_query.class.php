<?php
class USAM_Products_Document_Query 
{
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'document_id' => 'd', 'product_id' => 'd', 'name' => 's', 'price' => 'f', 'old_price' => 'f', 'quantity' => 'f', 'used_bonuses' => 'd', 'unit_measure' => 's', 'date_insert' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_products_document', $this );
		...
		{		
			if ( $field == 'all' )
				$this->fields[] = USAM_TABLE_DOCUMENT_PRODUCTS.".*";	
			elseif ( $field == 'count' )
			...
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_DOCUMENT_PRODUCTS.".$field";
		}
		if ( !count($this->fields) )
			$this->query_fields = USAM_TABLE_DOCUMENT_PRODUCTS.".*";
		else
		...
		$this->query_from = "FROM ".USAM_TABLE_DOCUMENT_PRODUCTS;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( [$date_parameters], USAM_TABLE_DOCUMENT_PRODUCTS );
			$this->query_where .= $date_query->get_sql();
		} 
		if ( !empty($qv['date_query']) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_DOCUMENT_PRODUCTS );
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_DOCUMENT_PRODUCTS.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			$search_columns = apply_filters( 'usam_products_document_search_columns', $search_columns, $search, $this );
		...
		{	// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_DOCUMENT_PRODUCTS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_DOCUMENT_PRODUCTS.".id NOT IN ($ids)";
		}						
		if ( isset($qv['document_id']) ) 
		{
			$document_id = implode( ',',  (array)$qv['document_id'] );
			$this->query_where .= " AND ".USAM_TABLE_DOCUMENT_PRODUCTS.".document_id IN ($document_id)";
		}						
		...
		if ( isset($qv['product_ids']) ) 
		{
			...
			$this->query_where .= " AND ".USAM_TABLE_DOCUMENT_PRODUCTS.".product_id IN ('$product_ids')";
		}		
		$conditions_query = new USAM_Conditions_Query();
		$this->query_where .= $conditions_query->get_sql_clauses( $qv );
		if ( $document_join )
			$this->query_join .= " INNER JOIN ".USAM_TABLE_DOCUMENTS." ON (".USAM_TABLE_DOCUMENTS.".id = ".USAM_TABLE_DOCUMENT_PRODUCTS.".document_id)";		
		do_action_ref_array( 'usam_pre_products_document_query', array( &$this ) );
	}
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_product_document' );
		return $data;
	}
	...
	public function query()
	{
		...
		if ( isset($qv['count_total'] ) && $qv['count_total'] )
			$this->total = (int)$wpdb->get_var( apply_filters( 'found_products_document_query', 'SELECT FOUND_ROWS()' ) );
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_DOCUMENT_PRODUCTS.'.'.$orderby;	 
		elseif ( 'date' == $orderby ) 
			$_orderby = USAM_TABLE_DOCUMENT_PRODUCTS.'.date_insert';
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) )
		{
			$include = wp_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".USAM_TABLE_DOCUMENT_PRODUCTS.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_products_document_query( $args )
{
	$args['count_total'] = false;	
	$query = new USAM_Products_Document_Query( $args );	
