...
{		
	...
	{ 		
		...
		$storages = usam_get_storages(['fields' => 'id', 'issuing' => 1, 'number' => 20, 'cache_meta' => true, 'cache_location' => true, 'location_id' => [0, $location_id], 'owner' => '']);		
		if ( $storages )
		{
			...
			<div class="stock_in_storages">
				...
				{
					$in_stock = isset($post->ID) && usam_get_stock_in_storage($storage_id, $post->ID)?1:0;		
					if ( $in_stock )
					{
						$product_not_available = false;
						?>
						<div class="stock_in_storage">	
							<div class="stock_in_storage__name"><span class="stock_in_storage__name_text"><?php echo usam_get_storage_metadata( $storage_id, 'address'); ?></span></div>
							<div class="stock_in_storage__stock <?php echo $in_stock?'usam_product_in_stock':'usam_product_not_available'; ?>"><?php echo usam_get_stock_in_storage($storage_id, $post->ID, 'short'); ?></div>
						</div>
						<?php
					}
				}
