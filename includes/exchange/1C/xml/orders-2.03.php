...
foreach ($orders as $order):
	...
	if ( usam_is_type_payer_company( $order->type_payer ) )
	{
		...
		$contragent_properties_1с = ['id' => 'Ид', 'role' => 'Роль', 'delete' => 'ПометкаУдаления', 'inn' => 'ИНН', 'ppc' => 'КПП', 'okpo' => 'ОКПО', 'name' => 'Наименование', 'official_name' => 'ОфициальноеНаименование', 'full_name' => 'ПолноеНаименование'];
		$contact_properties_1с = ['email' => 'Почта', 'phone' => 'Телефон', 'phonework' => 'ТелефонРабочий'];				
		$contragents['billing']['name'] = usam_get_order_metadata( $order->id, 'company' );
		$contragents['billing']['full_name'] = usam_get_order_metadata( $order->id, 'company' );
		...
		{										
			...
			$contragents['billing']['full_name'] = usam_get_company_metadata( $order->company_id, 'full_company_name');
		...
		$contragents['billing']['contact']['phonework'] = $contragents['billing']['contact']['phone'];		
		$contragents['billing']['official_name'] = $contragents['billing']['full_name'];
		if( !empty($contragents['shipping']) )
			$contragents['shipping']['official_name'] = $contragents['billing']['full_name'];
	}	
	else
	{ 
		$contragent_properties_1с = ['id' => 'Ид', 'role' => 'Роль', 'delete' => 'ПометкаУдаления', 'firstname' => 'Имя', 'lastname' => 'Фамилия', 'patronymic' => 'Отчество', 'name' => 'Наименование'];
		$contact_properties_1с = ['email' => 'Почта', 'phone' => 'Телефон', 'phonework' => 'ТелефонРабочий'];
		...
		{	
			...
			$contragents['billing']['patronymic'] = '';			
		...
		{										
			...
			$contragents['shipping']['patronymic'] = '';	
			$contragents['shipping']['role'] = "Получатель";					
			$properties_adress_1с = ['postcode' => 'shippingpostcode', 'location' => 'shippinglocation', 'address' => 'shippingaddress'];	
			$contragents['shipping']['address'] = usam_get_crm_address( $order->id, $properties_adress_1с );													
	...
	$properties_1с = ['Ид' => $order->id, 'Номер' => $order->number, 'Дата' => usam_local_date($order->date_insert, 'Y-m-d'), 'Время' => usam_local_date($order->date_insert, 'H:i:s'), 'ПометкаУдаления' => $order->status=='delete'?'true':'false', 'ХозОперация' => 'Заказ товара', 'Источник' => 'Заказ с сайта', 'Роль' => 'Продавец', 'Валюта' => $currency, 'Курс' => 1, 'Сумма' => $order->totalprice, 'Комментарий' => usam_utf8_for_xml( $note ) ];
...
<Документ>
		...
		foreach ($contragents as $type => $contragent):
			...
			<Контрагент>			
				...
				foreach ($contragent as $property_type => $value)
				{
					...
					{
						?><<?php echo $contragent_properties_1с[$property_type]; ?>><?php echo stripcslashes(htmlspecialchars($value)); ?></<?php echo $contragent_properties_1с[$property_type]; ?>><?php 
					}
				}				
				if ( !empty($bank_accounts) )
				{
					...
					?>
						<РасчетныйСчет>
							...
							<Банк>
								<Наименование><?php echo stripcslashes(htmlspecialchars($account->name)); ?></Наименование>
				...
				if ( !empty($contragent['contact_person']) )
				{
					...
					<Представители>
						<Представитель>
							...
							<Наименование><?php echo stripcslashes(htmlspecialchars($contragent['contact_person'])) ?></Наименование>
						</Представитель>
					</Представители>					
					<?php 
				}
				  ...
				  <АдресРегистрации>  
					...
					foreach ($contragent['legaladdress'] as $address_item_name => $address_item_value): 
						...
						<АдресноеПоле>								
							...
							<Значение><?php echo stripcslashes(htmlspecialchars($address_item_value)); ?></Значение>
						 </АдресноеПоле>
						<?php } ?>
					<?php endforeach ?>
				  </АдресРегистрации>
				...
				{
					if ( !$value )
						unset($contragent['contact'][$name]);
				}				
				if (!empty($contragent['contact']) ): ?>
					<Контакты>
						<?php 
						foreach ($contragent['contact'] as $name => $value): 
						...
						?>
						<Контакт>
							<Тип><?php echo stripcslashes(htmlspecialchars($contact_properties_1с[$name])); ?></Тип>
							<Значение><?php echo stripcslashes(htmlspecialchars($value)); ?></Значение>
						</Контакт>
						<?php endforeach ?>					
					</Контакты>	
				<?php endif ?>		
	...
	<ЗначенияРеквизитов>
		...
		<ЗначениеРеквизита>
			...
			<Значение><?php echo stripcslashes($payment['name']) ?></Значение>
		</ЗначениеРеквизита>
		<ЗначениеРеквизита>
	...
	<Товары>
		...
		if ( !$document->storage_pickup ) {
			...
			<Товар>					
				...
				<Наименование><?php echo htmlspecialchars($document->name); ?></Наименование>				
				<ЗначенияРеквизитов>								
					<ЗначениеРеквизита>
		...
		foreach ($products as $product): 
			...
			<Товар>
				...
				<Наименование><?php echo stripcslashes(htmlspecialchars($product->name)); ?></Наименование>				
				<ЗначенияРеквизитов>
					<ЗначениеРеквизита>
						<Наименование>ВидНоменклатуры</Наименование>
