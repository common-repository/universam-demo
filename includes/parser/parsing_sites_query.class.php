<?php
// Продавцы
class USAM_Parsing_Sites_Query 
{
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'name' => 's', 'site_type' => 's', 'type_price' => 's', 'store' => 's', 'scheme' => 's', 'view_product' => 's', 'domain' => 's', 'active' => 'd', 'proxy' => 's', 'count_urls' => 'd', 'start_date' => 's', 'end_date' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_parsing_sites', $this );
		...
		{
			if ( $field == 'all' )
				$this->fields[] = USAM_TABLE_PARSING_SITES.".*";		
			elseif ( $field == 'count' )
				$this->fields[] = "COUNT(*) AS count";								
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_PARSING_SITES.'.'.$field;
		}
		...
		$this->query_from = "FROM ".USAM_TABLE_PARSING_SITES;
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_PARSING_SITES.".id $order";
		}
		...
		if ( $search ) {
			...
			$search_columns = apply_filters( 'usam_parsing_sites_search_columns', $search_columns, $search, $this );
			$this->query_where .= $this->get_search_sql( $search, $search_columns, $wild );
		}
		if ( !empty( $include ) ) 
		{
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_PARSING_SITES.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_PARSING_SITES.".id NOT IN ($ids)";
		}  	
		if ( !empty($qv['site_type']) ) 
		{		
			$site_type = implode( "','", (array)$qv['site_type'] );		
			$this->query_where .= " AND ".USAM_TABLE_PARSING_SITES.".site_type IN ('$site_type')";
		}	
		if ( isset($qv['active']) && $qv['active'] != 'all' ) 
		{		
			$active = $qv['active'] ? 1 : 0;		
			$this->query_where .= " AND ".USAM_TABLE_PARSING_SITES.".active=$active";
		}		
		do_action_ref_array( 'usam_pre_parsing_sites_query', array( &$this ) );
	}
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_parsing_site' );	
		return $data;
	}
	...
	public function query()
	{
		...
		if ( $count_fields > 1 || $qv['fields'] == 'all' && $qv['number'] != 1 || 'domain=>name' == $qv['fields'] || 'id=>name' == $qv['fields']) 
		{
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		{
			if ( 'id' == $col )
				$searches[] = USAM_TABLE_PARSING_SITES.'.'.$wpdb->prepare( "$col = %s", $string );
			elseif ( 'name' == $col )
			{
				$searches[] = USAM_TABLE_PARSING_SITES.'.'.$wpdb->prepare( "$col LIKE %s", $like );
			}
		}
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_PARSING_SITES.'.'.$orderby;
		elseif ( 'meta_value' == $orderby ) 
			$_orderby = USAM_TABLE_PARSING_SITE_META.".meta_value";
		elseif ( 'meta_value_num' == $orderby ) 
			$_orderby = "CAST(".USAM_TABLE_PARSING_SITE_META.".meta_value AS signed)";
		...
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) ) 
		{
			...
			$_orderby = "FIELD( ".USAM_TABLE_PARSING_SITES.".id, $include_sql )";
		} 
		return $_orderby;
	}
...
function usam_get_parsing_sites( $query = array() )
{	
	$query['count_total'] = false; 
	$parsing_sites = new USAM_Parsing_Sites_Query( $query );	
	return $parsing_sites->get_results();	
}