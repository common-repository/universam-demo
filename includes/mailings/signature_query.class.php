...
class USAM_Signatures_Query 
{
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_signature', $this );
		...
		if ( 'all' == $qv['fields'] ) 		
			$this->query_fields = USAM_TABLE_SIGNATURES.".*";
		else
		{
			...
			{	
				...
				else
				{
					...
					$this->query_fields[] = USAM_TABLE_SIGNATURES.".$field";
				}
			}
		...
		$this->query_from = "FROM ".USAM_TABLE_SIGNATURES;
		...
		if ( !empty( $mailbox_id ) ) 
		{ 
			$this->query_where .= " AND ".USAM_TABLE_SIGNATURES.".mailbox_id IN ('".implode( "','",  $mailbox_id )."')";		
		}		
		if ( !empty($mailbox_id__not_in) ) 
		{			
			$this->query_where .= " AND ".USAM_TABLE_SIGNATURES.".mailbox_id NOT IN ('".implode( "','",  $mailbox_id__not_in )."')";
		}
		if ( !empty( $mailbox_id__in ) ) 
		{			
			$this->query_where .= " AND ".USAM_TABLE_SIGNATURES.".mailbox_id IN ('".implode( "','",  $mailbox_id__in )."')";
		}				
		// Группировать
		...
		foreach ( $ordersby as $_key => $_value ) 
		{
			...
			$orderby_array[] = USAM_TABLE_SIGNATURES.'.'.$parsed . ' ' . $this->parse_order( $_order );
		}

		// If no valid clauses were found, order by id.
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_SIGNATURES.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			if ( $qv['search_columns'] )			
				$search_columns = array_intersect( $qv['search_columns'], array( 'id', 'signature' ) );
			if ( ! $search_columns ) 
			{
				if ( is_numeric($search) )
					$search_columns = array('id', );				
				else
					$search_columns = array('signature' );
			}	
			$search_columns = apply_filters( 'usam_signature_search_columns', $search_columns, $search, $this );
		...
		{	// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_SIGNATURES.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_SIGNATURES.".id NOT IN ($ids)";
		}
		if( !empty($qv['manager_id']) ) 
		{
			if( $qv['manager_id'] == 'my' ) 
				$qv['manager_id'] = [ get_current_user_id() ];
			$ids = implode( ',', wp_parse_id_list( $qv['manager_id'] ) );
			$this->query_where .= " AND ".USAM_TABLE_SIGNATURES.".manager_id IN ($ids)";
		}	
		do_action_ref_array( 'usam_pre_signature_query', array( &$this ) );
	}
	...
	public function query()
	{
		...
		if ( isset($qv['count_total'] ) && $qv['count_total'] )
			$this->total = (int)$wpdb->get_var( apply_filters( 'found_signature_query', 'SELECT FOUND_ROWS()' ) );		
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		$_orderby = '';		
		if ( in_array( $orderby, array( 'id', 'mailbox_id', 'mailbox_id' ) ) )
		{
			$_orderby = $orderby;
		} 		
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) )
		{
			...
			$_orderby = "FIELD( ".USAM_TABLE_SIGNATURES.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_signatures( $args = array() )
{
	$args['count_total'] = false;	
	$class = new USAM_Signatures_Query( $args );	
