...
class USAM_Bonus_Cards_Query 
{
	...
	private $fields = [];
	private $db_fields = ['code' => 'd', 'status' => 's', 'user_id' => 'd', 'sum' => 'd', 'percent' => 'f', 'date_insert' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_bonus_cards', $this );
		...
		{
			if ( $field == 'all' || $field == 'user_id=>data' )
				$this->fields[] = USAM_TABLE_BONUS_CARDS.".*";
			elseif ( $field == 'appeal' )
			{
			...
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_BONUS_CARDS.".$field";			
		}
		if ( !count($this->fields) )
			$this->query_fields = USAM_TABLE_BONUS_CARDS.".*";
		else
		...
		$this->query_from = "FROM ".USAM_TABLE_BONUS_CARDS;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( array( $date_parameters ), USAM_TABLE_BONUS_CARDS );
		...
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_BONUS_CARDS );
		...
		if ( !empty($qv['status']) ) 
		{
			...
			if ( !empty($status) ) 
				$this->query_where .= " AND ".USAM_TABLE_BONUS_CARDS.".status IN ('".implode( "','", $status )."')";	
		}	
		if ( isset($qv['status__not_in']) ) 
		{
			$status__not_in = (array) $qv['status__not_in'];
			$this->query_where .= " AND ".USAM_TABLE_BONUS_CARDS.".status NOT IN ('".implode("','", $status__not_in )."')";
		}		
		// Группировать
		...
		{
			$orderby_array[] = USAM_TABLE_BONUS_CARDS.".code $order";
		}
		...
		if ( $search ) 
		{
			...
			$default = ['code', 'user_login', 'email', 'phone', 'full_name'];
			$search_columns = [];
			...
			{
				if ( strlen("$search") == 11 )
					$search_columns = array_intersect($search_columns, ['phone', 'code', 'user_login']);
				else
					$search_columns = array_intersect($search_columns, ['code', 'user_login']);
			}
			$search_columns = apply_filters( 'usam_bonus_cards_search_columns', $search_columns, $search, $this );
			$this->query_where .= $this->get_search_sql( $search, $search_columns, $wild );
		}
		if ( !empty( $include ) ) 
		{
			$ids = implode( ',', $include );			
			$this->query_where .= " AND ".USAM_TABLE_BONUS_CARDS.".code IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_BONUS_CARDS.".code NOT IN ($ids)";
		}	
		if ( !empty( $qv['user_id'] ) )
		{		
			$ids = (array)$qv['user_id'];
			$user_id = implode( ',', wp_parse_id_list( $ids ) );
			$this->query_where .= " AND ".USAM_TABLE_BONUS_CARDS.".user_id IN ($user_id)";
		}
		if ( !empty($qv['contact_source']) )
		{			
			$this->contact_join = true;		
			$this->query_where .= " AND ".USAM_TABLE_CONTACTS.".contact_source IN ('".implode( "','", (array)$qv['contact_source'] )."')";
		}	
		$conditions_query = new USAM_Conditions_Query();
		$this->query_where .= $conditions_query->get_sql_clauses( $qv );
		
		if ( $this->contact_join  ) 
			$this->query_join .= " INNER JOIN ".USAM_TABLE_CONTACTS." ON (".USAM_TABLE_BONUS_CARDS.".user_id = ".USAM_TABLE_CONTACTS.".user_id)";
		if ( $this->distinct ) 
			$this->query_fields = 'DISTINCT ' . $this->query_fields;
		
		do_action_ref_array( 'usam_pre_bonus_cards_query', array( &$this ) );
	}
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_bonus_card' );	
		return $data;
	}
	...
	public function query()
	{
		...
		$r = array();
		if( 'user_id=>data' == $qv['fields'] ) 
		{					
			foreach ($this->results as $key => $result ) 
				$r[$result->user_id] = $result;
			$this->results = $r;
		}
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		foreach ( $cols as $col ) 
		{
			if ( 'code' == $col ) 
			{
				$searches[] = $wpdb->prepare( USAM_TABLE_BONUS_CARDS.".$col = %s", $string );
			} 	
			elseif ( 'full_name' == $col ) 
			{				
				$this->distinct = true;	
				$this->query_join .= " LEFT JOIN ".USAM_TABLE_CONTACTS." ON (".USAM_TABLE_BONUS_CARDS.".user_id = ".USAM_TABLE_CONTACTS.".user_id)";
			...
			elseif ( 'phone' == $col ) 
			{
				...
				if ( !empty($user_id) )
					$searches[] = USAM_TABLE_BONUS_CARDS.".user_id ={$user_id}";
			}	
			elseif ( 'email' == $col ) 
			{
				$user = get_user_by('email', $string );
				if ( !empty($user->ID) )
					$searches[] = USAM_TABLE_BONUS_CARDS.".user_id ={$user->ID}";
			}			
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_BONUS_CARDS.'.'.$orderby;
		elseif ( $orderby == 'date' )
			$_orderby = USAM_TABLE_BONUS_CARDS.'.date_insert';
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) ) {
			$include = wp_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".USAM_TABLE_BONUS_CARDS.".code, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_bonus_cards( $query = array() )
{	
	$query['count_total'] = false;
	$bonus_cards = new USAM_Bonus_Cards_Query( $query );	
	return $bonus_cards->get_results();	
}	