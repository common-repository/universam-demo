<?php
class USAM_User_Referrals_Links_Query 
{
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_user_referral_links', $this );
		...
		if ( is_array( $qv['fields'] ) ) 
		{
			...
			{		
				...
				else
				{
					...
					$this->query_fields[] = USAM_TABLE_OPEN_REFERRAL_LINKS.".$field";
				}
			}
			$this->query_fields = implode( ',', $this->query_fields );
		} 
		elseif ( 'all' == $qv['fields'] ) 		
			$this->query_fields = USAM_TABLE_OPEN_REFERRAL_LINKS.".*";		
		else 	
			$this->query_fields = USAM_TABLE_OPEN_REFERRAL_LINKS.".id";
		...
		$this->query_from = "FROM ".USAM_TABLE_OPEN_REFERRAL_LINKS;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( array( $date_parameters ), USAM_TABLE_OPEN_REFERRAL_LINKS );
		...
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_OPEN_REFERRAL_LINKS );
		...
		foreach ( $ordersby as $_key => $_value ) 
		{
			...
			$orderby_array[] = USAM_TABLE_OPEN_REFERRAL_LINKS.'.'.$parsed . ' ' . $this->parse_order( $_order );
		}		
		if ( empty( $orderby_array ) ) 
		{
			$orderby_array[] = USAM_TABLE_OPEN_REFERRAL_LINKS.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			$search_columns = apply_filters( 'usam_user_referral_links_search_columns', $search_columns, $search, $this );
			$this->query_where .= $this->get_search_sql( $search, $search_columns, $wild );
		}
		if ( !empty( $include ) ) 
		{
			$ids = implode( ',', $include );			
			$this->query_where .= " AND ".USAM_TABLE_OPEN_REFERRAL_LINKS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_OPEN_REFERRAL_LINKS.".id NOT IN ($ids)";
		}	
		if ( !empty($qv['user_id']) )
		{		
			$ids = (array)$qv['user_id'];
			$user_id = implode( ',', wp_parse_id_list( $ids ) );
			$this->query_where .= " AND ".USAM_TABLE_OPEN_REFERRAL_LINKS.".user_id IN ($user_id)";
		}	
		if ( !empty($qv['contact_id']) )
		{		
			$ids = (array)$qv['contact_id'];
			$contact_id = implode( ',', wp_parse_id_list( $ids ) );
			$this->query_where .= " AND ".USAM_TABLE_OPEN_REFERRAL_LINKS.".contact_id IN ($contact_id)";
		}
		$conditions_query = new USAM_Conditions_Query();
		$this->query_where .= $conditions_query->get_sql_clauses( $qv );
		
		do_action_ref_array( 'usam_pre_user_referral_links_query', array( &$this ) );
	}
	
	
	public function handling_data_types( $data )
	{
		foreach (['id', 'user_id', 'contact_id'] as $column ) 
		{			
	...
	public function query()
	{
		...
		{ 
			if ( $qv['number'] == 1 )
				wp_cache_set( $this->results->id, (array)$this->results, 'usam_user_referral_link' );	
			else
			{					
				foreach ( $this->results as $result ) 
				{
					wp_cache_set( $result->id, (array)$result, 'usam_user_referral_link' );						
				}
			}			
		}			
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		{
			if ( 'id' == $col ) 
				$searches[] = $wpdb->prepare( USAM_TABLE_OPEN_REFERRAL_LINKS.".$col = %s", $string );
			else 
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) ) {
			...
			$_orderby = "FIELD( ".USAM_TABLE_OPEN_REFERRAL_LINKS.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_user_referrals_links( $query = [] )
{	
	$query['count_total'] = false;
	$user_referrals = new USAM_User_Referrals_Links_Query( $query );	
