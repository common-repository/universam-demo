<?php
// Класс работы с смс сообщениями
class USAM_SMS_Query 
{
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'phone' => 's', 'message' => 's', 'server_message_id' => 's', 'date_insert' => 's', 'folder' => 's', 'sent_at' => 's', 'user_id' => 'd'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_sms', $this );
		...
		{			
			if ( $field == 'all' )
				$this->fields[] = USAM_TABLE_SMS.".*";
			elseif ( $field == 'count' )
				$this->fields[] = "COUNT(*) AS count";	
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_SMS.".$field";			
		}
		if ( !count($this->fields) )
			$this->query_fields = USAM_TABLE_SMS.".*";
		else
		...
		$this->query_from = "FROM ".USAM_TABLE_SMS;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( [$date_parameters], USAM_TABLE_SMS );
			$this->query_where .= $date_query->get_sql();
		}
		if ( !empty($qv['date_query']) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_SMS );
		...
		if ( !empty($folder) ) 
		{
			$this->query_where .= " AND ".USAM_TABLE_SMS.".folder IN ('".implode( "','",  $folder )."')";		
		}		
		if ( !empty($folder_not_in) ) 
		{			
			$this->query_where .= " AND ".USAM_TABLE_SMS.".folder NOT IN ('".implode( "','",  $folder_not_in )."')";
		}			
		// Группировать
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_SMS.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			if ( $qv['search_columns'] )			
				$search_columns = array_intersect( $qv['search_columns'], array( 'id', 'phone', 'server_message_id' ) );
			if ( ! $search_columns ) 
			{
				if ( is_numeric($search) )
					$search_columns = array('id', 'phone' );				
				else
					$search_columns = array( 'server_message_id' );
			}	
			$search_columns = apply_filters( 'usam_sms_search_columns', $search_columns, $search, $this );
		...
		if ( !empty( $qv['include'] ) )
		{
			...
			$this->query_where .= " AND ".USAM_TABLE_SMS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_SMS.".id NOT IN ($ids)";
		}		
				
		if ( !empty( $qv['read'] ) ) 
		{			
			$read = (int)$qv['read'];
			$this->query_where .= " AND ".USAM_TABLE_SMS.".read=$read";
		}			
		if ( !empty( $qv['phone'] ) ) 
		{
			$phone = is_array($qv['phone']) ? $qv['phone']: array( $qv['phone'] );		
			$phone = array_map('intval', $phone);
			$this->query_where .= " AND ".USAM_TABLE_SMS.".phone IN (".implode( ",", $phone ).")";
		}			
		if ( !empty($qv['user_id']) ) 
		{
			...
			if ( !empty($user_id) ) 
				$this->query_where .= " AND ".USAM_TABLE_SMS.".user_id IN (".implode( ",", $user_id ).")";	
		}	
		if ( isset($qv['user_id__not_in']) ) 
		{
			$user_id__not_in = (array) $qv['user_id__not_in'];
			$this->query_where .= " AND ".USAM_TABLE_SMS.".user_id NOT IN (".implode(",", $user_id__not_in ).")";
		}
		do_action_ref_array( 'usam_pre_sms_query', array( &$this ) );
	}
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_sms' );	
		return $data;
	}
	...
	public function query()
	{
		...
		if ( isset($qv['count_total'] ) && $qv['count_total'] )
			$this->total = (int)$wpdb->get_var( apply_filters( 'found_sms_query', 'SELECT FOUND_ROWS()' ) );			
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		foreach ( $cols as $col ) {
			if ( 'id' == $col ) {
				$searches[] = $wpdb->prepare( USAM_TABLE_SMS.".$col = %s", $string );
			} else {
				$searches[] = $wpdb->prepare( USAM_TABLE_SMS.".$col LIKE %s", $like );
			}
		}
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )		
			$_orderby = USAM_TABLE_SMS.'.'.$orderby;
		elseif ( 'date' == $orderby ) 
			$_orderby = USAM_TABLE_SMS.'.date_insert';
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) )
		{
			$include = wp_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".USAM_TABLE_SMS.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_sms_query( $args )
{
	$args['count_total'] = false;
	$sms = new USAM_SMS_Query( $args );	
	$result = $sms->get_results();		
	return $result;
}

function usam_get_number_sms_messages( )
{
	global $wpdb;	
	
	$results = $wpdb->get_results("SELECT COUNT(*) AS count, folder FROM `".USAM_TABLE_SMS."` GROUP BY folder");	
	
	$numbers = array();
	$total = 0;
	foreach ( $results as $result )
	{
		$numbers[$result->folder] = $result->count;
		$total += $result->count;
