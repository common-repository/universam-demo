...
class USAM_Chat_Bot_Commands_Query 
{
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_chat_bot_commands', $this );
		...
		if ( is_array( $qv['fields'] ) ) 
		{
			...
			{	
				if ( $field == 'all' )
					$this->query_fields[] = USAM_TABLE_CHAT_BOT_COMMANDS.".*";	
				else
				{
					$field = 'id' === $field ? 'id' : sanitize_key( $field );						
					$this->query_fields[] = USAM_TABLE_CHAT_BOT_COMMANDS.".$field";
				}
			}
		...
		elseif ( 'all' == $qv['fields'] ) 		
			...
			$this->query_fields = USAM_TABLE_CHAT_BOT_COMMANDS.".".$qv['fields'];
		...
		$this->query_from = "FROM ".USAM_TABLE_CHAT_BOT_COMMANDS;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( array( $date_parameters ), USAM_TABLE_CHAT_BOT_COMMANDS );
		...
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_CHAT_BOT_COMMANDS );
		...
		foreach ( $ordersby as $_key => $_value ) 
		{
			...
			$orderby_array[] = USAM_TABLE_CHAT_BOT_COMMANDS.'.'.$parsed . ' ' . $this->parse_order( $_order );
		}

		// If no valid clauses were found, order by id.
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_CHAT_BOT_COMMANDS.".id $order";
		}
		...
		if ( $search ) {
			...
			$search_columns = apply_filters( 'usam_chat_bot_commands_search_columns', $search_columns, $search, $this );
		...
		if ( !empty( $include ) ) 
		{
			// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_CHAT_BOT_COMMANDS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_CHAT_BOT_COMMANDS.".id NOT IN ($ids)";
		}  	
		if ( !empty($qv['template_id']) ) 
		{		
			$template_id = implode( ',',  wp_parse_id_list( (array)$qv['template_id'] ) );		
			$this->query_where .= " AND ".USAM_TABLE_CHAT_BOT_COMMANDS.".template_id IN ($template_id)";
		}
		if ( isset($qv['active']) ) 
		{		
			$active = $qv['active']?1:0;		
			$this->query_where .= " AND ".USAM_TABLE_CHAT_BOT_COMMANDS.".active=$active";
		}			
		do_action_ref_array( 'usam_pre_chat_bot_commands_query', array( &$this ) );
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		{
			if ( 'id' == $col )
				$searches[] = USAM_TABLE_CHAT_BOT_COMMANDS.'.'.$wpdb->prepare( "$col = %s", $string );
			elseif ( 'message' == $col )
			{
				$searches[] = USAM_TABLE_CHAT_BOT_COMMANDS.'.'.$wpdb->prepare( "$col LIKE %s", $like );
			}
		}
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) ) 
		{
			...
			$_orderby = "FIELD( ".USAM_TABLE_CHAT_BOT_COMMANDS.".id, $include_sql )";
		} 
		else 
		{
			$_orderby = $orderby;
...
function usam_get_chat_bot_commands( $query )
{	
	$query['count_total'] = false; 
	$chat_bot_commands = new USAM_Chat_Bot_Commands_Query( $query );	
	return $chat_bot_commands->get_results();	
}