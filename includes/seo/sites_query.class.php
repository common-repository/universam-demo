<?php
// Класс работы с сайтами конкурентами в поиске
class USAM_Sites_Query 
{
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'domain' => 's', 'description' => 's', 'type' => 's', 'date_insert' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_sites', $this );
		...
		{
			if ( $field == 'all' )
				$this->fields[] = USAM_TABLE_SITES.".*";		
			elseif ( $field == 'count' )
				$this->fields[] = "COUNT(*) AS count";					
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_SITES.'.'.$field;
		}
		...
		$this->query_from = "FROM ".USAM_TABLE_SITES;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( [$date_parameters], USAM_TABLE_SITES );
			$this->query_where .= $date_query->get_sql();
		}
		if ( !empty($qv['date_query']) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_SITES );
		...
		if ( !empty($qv['status']) ) 
		{
			...
			if ( !empty($status) ) 
				$this->query_where .= " AND ".USAM_TABLE_SITES.".status IN ('".implode("','",  $status)."')";	
		}	
		if ( isset($qv['status__not_in']) ) 
		{
			$status__not_in = (array) $qv['status__not_in'];
			$this->query_where .= " AND ".USAM_TABLE_SITES.".status NOT IN ('".implode("','",  $status__not_in)."')";
		}				
		// Группировать
		...
		if ( empty( $orderby_array ) ) {
			$orderby_array[] = USAM_TABLE_SITES.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			if ( $qv['search_columns'] )			
				$search_columns = array_intersect( $qv['search_columns'], array( 'id','domain' ) );
			if ( ! $search_columns ) 
			{
		...
		{	// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND ".USAM_TABLE_SITES.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_SITES.".id NOT IN ($ids)";
		}			
		if ( !empty( $qv['location_id'] ) ) 
		{		
			$location_id = implode( ',',  (array)$qv['location_id'] );		
			$this->query_where .= " AND ".USAM_TABLE_SITES.".id IN (SELECT site_id FROM ".USAM_TABLE_STATISTICS_KEYWORDS." WHERE location_id IN ($location_id))";
		}	
		if ( !empty( $qv['date_query_statistic'] ) ) 
		{			
			$date_query_statistic = new USAM_Date_Query( $qv['date_query_statistic'], USAM_TABLE_STATISTICS_KEYWORDS );
			$this->query_where .= " AND ".USAM_TABLE_SITES.".id IN (SELECT site_id FROM ".USAM_TABLE_STATISTICS_KEYWORDS." WHERE 1=1".$date_query_statistic->get_sql().")";
		}		
		do_action_ref_array( 'usam_pre_sites_query', array( &$this ) );
	}
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_site' );	
		return $data;
	}
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_SITES.'.'.$orderby;
		elseif ( $orderby == 'date' )
			$_orderby = USAM_TABLE_SITES.'.date_insert';	
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) )
		{
			$include = wp_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".USAM_TABLE_SITES.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_sites( $args )
{
	$args['count_total'] = false;	
	$payments = new USAM_Sites_Query( $args );	
