...
class USAM_Calls_Query 
{
	...
	public function prepare_query( $query = array() ) 
	{
		...
		if ( is_array( $qv['fields'] ) ) 
		{
			...
			{		
				...
				elseif ( $field == 'sum_time' )
					$this->query_fields[] = "SUM(time) AS sum_time";			
				else					
				{
					$field = 'id' === $field ? 'id' : sanitize_key( $field );						
					$this->query_fields[] = USAM_TABLE_CALLS.".$field";
				}
			}
			$this->query_fields = implode( ',', $this->query_fields );
		} 
		elseif ( 'sum_time' == $qv['fields'] ) 		
			$this->query_fields = "SUM(time) AS sum_time";			
		elseif ( 'all' == $qv['fields'] ) 		
			$this->query_fields = USAM_TABLE_CALLS.".*";		
		else 	
			$this->query_fields = USAM_TABLE_CALLS.".id";
		...
		$this->query_from = "FROM ".USAM_TABLE_CALLS;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( array( $date_parameters ), USAM_TABLE_CALLS );
		...
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_CALLS );
		...
		if ( !empty($qv['status']) ) 
		{
			...
			if ( !empty($status) ) 
				$this->query_where .= " AND ".USAM_TABLE_CALLS.".status IN ('".implode( "','", $status )."')";	
		}	
		if ( isset($qv['status__not_in']) ) 
		{
			$status__not_in = (array) $qv['status__not_in'];
			$this->query_where .= " AND ".USAM_TABLE_CALLS.".status NOT IN ('".implode("','", $status__not_in )."')";
		}
		// Группировать
		...
		foreach ( $ordersby as $_key => $_value ) 
		{
			...
			$orderby_array[] = USAM_TABLE_CALLS.'.'.$parsed . ' ' . $this->parse_order( $_order );
		}		
		if ( empty( $orderby_array ) ) 
		{
			$orderby_array[] = USAM_TABLE_CALLS.".id $order";
		}
		...
		{
			$ids = implode( ',', $include );			
			$this->query_where .= " AND ".USAM_TABLE_CALLS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_CALLS.".id NOT IN ($ids)";
		}	
		if ( !empty( $qv['manager_id'] ) )
		{		
			$ids = (array)$qv['manager_id'];
			$manager_id = implode( ',', wp_parse_id_list( $ids ) );
			$this->query_where .= " AND ".USAM_TABLE_CALLS.".manager_id IN ($manager_id)";
		}	
		if ( !empty( $qv['call_id'] ) )
		{		
			$call_id = (array)$qv['call_id'];
			$this->query_where .= " AND ".USAM_TABLE_CALLS.".call_id IN ('".implode( "','", $call_id )."')";
		}		
		if ( !empty( $qv['phone'] ) )
		{		
			$phone = (array)$qv['phone'];
			$this->query_where .= " AND ".USAM_TABLE_CALLS.".phone IN ('".implode( "','", $phone )."')";
		}			
	...
	public function query()
	{
		...
		{ 
			if ( $qv['number'] == 1 )
				wp_cache_set( $this->results->id, (array)$this->results, 'usam_call' );	
			else
			{					
				foreach ( $this->results as $result ) 
				{
					wp_cache_set( $result->id, (array)$result, 'usam_call' );						
				}
			}			
		}			
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		foreach ( $cols as $col ) 
		{
			if ( 'id' == $col ) 
			{
				$searches[] = $wpdb->prepare( USAM_TABLE_CALLS.".$col = %s", $string );
			} 			
			else 
			{
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		$_orderby = '';
		if ( in_array( $orderby, ['id', 'message', 'status']) )
		{
			$_orderby = $orderby;
		} 			
		elseif ( 'date' == $orderby ) 
		...
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) ) 
		{
			...
			$_orderby = "FIELD( ".USAM_TABLE_CALLS.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_calls( $query_vars = array() )
{	
	$query_vars['count_total'] = false;
	$query = new USAM_Calls_Query( $query_vars );	
