...
class USAM_Events_Query 
{
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'user_id' => 'd', 'type' => 's', 'calendar' => 'd', 'title' => 's', 'description' => 's', 'date_insert' => 's', 'date_completion' => 's', 'color' => 's', 'schedule' => 's', 'start' => 's', 'end' => 's', 'importance' => 'd', 'status' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		foreach ( $fields as $field ) 
		{
			...
			elseif ( $field == 'last_comment' )	
			{
				...
				$join[] = " LEFT JOIN ".USAM_TABLE_COMMENTS." AS comment ON (comment.object_id=".USAM_TABLE_EVENTS.".id AND comment.object_type='event' AND comment.id = (SELECT id FROM ".USAM_TABLE_COMMENTS." WHERE object_type='event' AND status!=1 AND object_id=".USAM_TABLE_EVENTS.".id ORDER BY id DESC LIMIT 1 ))";
			}
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_EVENTS.".$field";
		}
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( [$date_parameters], USAM_TABLE_EVENTS );
			$this->query_where .= $date_query->get_sql();
		}
		if ( !empty($qv['date_query']) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_EVENTS );
		...
		{
			$this->query_join .= " INNER JOIN ".USAM_TABLE_OBJECT_STATUSES." ON (".USAM_TABLE_OBJECT_STATUSES.".internalname=".USAM_TABLE_EVENTS.".status)";			
			$events = usam_get_events_types();
			if ( $qv['status_type'] == 'unclosed' )
				$this->query_where .= " AND ".USAM_TABLE_OBJECT_STATUSES.".close=0 AND ".USAM_TABLE_OBJECT_STATUSES.".type IN ('".implode("','", array_keys($events))."')";
			elseif ( $qv['status_type'] == 'closed' )
				$this->query_where .= " AND ".USAM_TABLE_OBJECT_STATUSES.".close=1 AND ".USAM_TABLE_OBJECT_STATUSES.".type IN ('".implode("','", array_keys($events))."')";
			$distinct = true;
		}
		if ( !empty($qv['status']) ) 
		{
		...
		if ( !empty($type__not_in) ) 
			$this->query_where .= " AND ".USAM_TABLE_EVENTS.".type NOT IN ('".implode( "','",  $type__not_in )."')";
		// Группировать
			...
			{				
				switch ( $_value ) 
				{
					...
					case 'start_day' :
						if ( USAM_SQL_TIME_ZONE )
						{
							$ordersby_array[] = "DAY(CONVERT_TZ(start,'UTC','$name_timezone')))";
							$ordersby_array[] = "MONTH(CONVERT_TZ(start,'UTC','$name_timezone')))";
							$ordersby_array[] = "YEAR((CONVERT_TZ(start,'UTC','$name_timezone')))";						
						}
						else
						{
							$ordersby_array[] = "DAY(start)";
							$ordersby_array[] = "MONTH(start)";
							$ordersby_array[] = "YEAR(start)";								
						}
					break;
					case 'start_week' :
						if ( USAM_SQL_TIME_ZONE )
						{
							$ordersby_array[] = "WEEKOFYEAR(CONVERT_TZ(start,'UTC','$name_timezone'))";
							$ordersby_array[] = "YEAR(CONVERT_TZ(start,'UTC','$name_timezone'))";						
						}
						else
						{
							$ordersby_array[] = "WEEKOFYEAR(start)";
							$ordersby_array[] = "YEAR(start)";		
						}										
					break;	
					case 'start_month' :					
		...
		if ( $search ) 
		{
			...
			if ( $qv['search_columns'] )			
				$search_columns = array_intersect( $qv['search_columns'], ['id', 'user_login', 'calendar', 'title', 'description']);
			if ( ! $search_columns ) 
			{
				if ( is_numeric($search) )
					$search_columns = ['id', 'user_login', 'title'];				
				else
					$search_columns = ['title', 'user_login', 'description'];
			}	
		...
		if ( !empty($qv['author']) ) 
		{
			$author_ids = is_array($qv['author']) ? $qv['author']: array( $qv['author'] );		
			$author_ids = implode( ',',  (array)$author_ids );
			$this->query_where .= " AND ".USAM_TABLE_EVENTS.".user_id IN ($author_ids)";
		}		
		if ( !empty($qv['user_work']) ) 
		{			
			$user_ids = is_array($qv['user_work']) ? $qv['user_work']: array( $qv['user_work'] );			
			$user_ids = array_map('intval', $user_ids);			
			
			$this->query_join .= " LEFT JOIN ".USAM_TABLE_EVENT_USERS." ON (".USAM_TABLE_EVENTS.".id = ".USAM_TABLE_EVENT_USERS.".event_id )";						
			$user_where = " (".USAM_TABLE_EVENTS.".user_id IN (".implode(",", $user_ids).") OR ".USAM_TABLE_EVENT_USERS.".user_id IN (".implode(",", $user_ids).") )";
			if ( !empty($type) && in_array('project', $type) ) 
				$user_where .= " OR ".USAM_TABLE_EVENTS.".type ='project'";
			$this->query_where .= " AND ($user_where)";
			$distinct = true;			
		}
		if ( !empty($qv['users']) && is_array($qv['users']) ) 
		{
			$sql = [];
			foreach ( $qv['users'] as $code => $user_ids )
			{
				$user_ids = array_map('intval', (array)$user_ids);
				$sql[] = USAM_TABLE_EVENT_USERS.".user_id IN (".implode(",", $user_ids).") AND ".USAM_TABLE_EVENT_USERS.".user_type='$code'";
			}
			$this->query_where .= " AND ".implode( " OR ", $sql )."";
			if( strripos($this->query_join, USAM_TABLE_EVENT_USERS) === false )
				$this->query_join .= " LEFT JOIN ".USAM_TABLE_EVENT_USERS." ON (".USAM_TABLE_EVENTS.".id = ".USAM_TABLE_EVENT_USERS.".event_id )";
		}	
		if ( !empty($qv['users__not_in']) && is_array($qv['users__not_in']) ) 
		{
			$sql = [];
			foreach ( $qv['users__not_in'] as $code => $user_ids )
			{
				$user_ids = array_map('intval', (array)$user_ids);
				$sql[] = USAM_TABLE_EVENT_USERS.".user_id NOT IN (".implode(",", $user_ids).") AND ".USAM_TABLE_EVENT_USERS.".user_type='$code'";
			}
			$this->query_where .= " AND ".implode( " OR ", $sql )."";
			if( strripos($this->query_join, USAM_TABLE_EVENT_USERS) === false )
				$this->query_join .= " LEFT JOIN ".USAM_TABLE_EVENT_USERS." ON (".USAM_TABLE_EVENTS.".id = ".USAM_TABLE_EVENT_USERS.".event_id )";
		...
		{
			$this->query_join .= " INNER JOIN ".USAM_TABLE_RIBBON." ON (".USAM_TABLE_RIBBON.".event_id=".USAM_TABLE_EVENTS.".id)";			
		...
		{
			$clauses = $this->meta_query->get_sql( 'event', USAM_TABLE_EVENT_META, USAM_TABLE_EVENTS, 'id', $this );
			$this->query_join .= $clauses['join'];
			$this->query_where .= $clauses['where']; 
		...
		if ( $group_join  ) 
			$this->query_join .= " INNER JOIN ".USAM_TABLE_GROUP_RELATIONSHIPS." ON (".USAM_TABLE_GROUP_RELATIONSHIPS.".object_id = ".USAM_TABLE_EVENTS.".id)";
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_event' );	
		return $data;
	}
	...
	public function query()
	{
		...
		if ( $qv['cache_objects'] || $qv['cache_actions'] || $qv['cache_meta'] || $qv['cache_comments'] || $qv['cache_contacts'] || $qv['cache_last_comment_contacts'] )
		{
			$ids = [];	
			...
			{				
				if ( $qv['cache_actions'] )
				{
					$action_lists = usam_get_event_action_lists(['event_id' => $ids]);					
					$cache = [];
					foreach( $action_lists as $item )
						$cache[$item->event_id][] = $item;						
					foreach( $this->results as $item ) 	
					{
						$items = isset($cache[$item->id])?$cache[$item->id]:[];
						wp_cache_set( $item->id, $items, 'usam_event_actions' );
					}
				}
			}
			if ( $qv['cache_meta'] )
				usam_update_cache( $ids, [USAM_TABLE_EVENT_META => 'event_meta'], 'event_id' );		
			if ( $qv['cache_contacts'] )		
			{
				$cache = usam_update_cache($ids, [USAM_TABLE_EVENT_USERS => 'event_users'], 'event_id');	
				$user_ids = array();	
				if ( !empty($cache) )
				{					
					foreach ( $ids as $id ) 
					{
						if ( !empty($cache['usam_event_users'][$id]) )
							foreach ( $cache['usam_event_users'][$id] as $user ) 
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		foreach ( $cols as $col )
		{
			if ( 'id' == $col ) 
			{
				$searches[] = USAM_TABLE_EVENTS.".".$wpdb->prepare( "$col = %s", $string );
			} 
			elseif ( 'title' == $col ||  'description' == $col ) 
			{				
				$searches[] = USAM_TABLE_EVENTS.".{$col} LIKE LOWER ('%".$wpdb->esc_like( $string )."%')";	
			} 			
			elseif ( 'user_login' == $col ) 
			{
				$user = get_user_by('login', $string );
				if ( !empty($user->ID) )
					$searches[] = USAM_TABLE_EVENTS.".user_id ={$user->ID}";
			}		
			else 
			{
				$searches[] = USAM_TABLE_EVENTS.".".$wpdb->prepare( "$col LIKE %s", $like );
			}
		}
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_EVENTS.'.'.$orderby;
		elseif ( 'comment_count' == $orderby ) 
		{
			$this->query_join .= "LEFT JOIN ".USAM_TABLE_COMMENTS." AS comment ON (comment.object_id=".USAM_TABLE_EVENTS.".id AND comment.object_type ='event' AND comment.status!=2)";
			$_orderby = 'IF(comment.id>0,1,0) DESC, COUNT(comment.id)';
			
			if ( empty($this->query_groupby) )
				$this->query_groupby = 'GROUP BY '.USAM_TABLE_EVENTS.'.id';
			else
				$this->query_groupby .= ','.USAM_TABLE_EVENTS.'.id';		
		} 
		...
		{
			$this->query_join .= "LEFT JOIN ".USAM_TABLE_COMMENTS." AS comment ON (comment.object_id=".USAM_TABLE_EVENTS.".id AND comment.object_type ='event' AND comment.status!=2)";			
			$_orderby = 'COALESCE(MAX(comment.date_insert), '.USAM_TABLE_EVENTS.'.date_insert)';			
			if ( empty($this->query_groupby) )
				$this->query_groupby = 'GROUP BY '.USAM_TABLE_EVENTS.'.id';
			else
				$this->query_groupby .= ','.USAM_TABLE_EVENTS.'.id';		
...
function usam_get_events( $args = array() )
{ 
	$args['count_total'] = false;	
	$tasks = new USAM_Events_Query( $args );	
