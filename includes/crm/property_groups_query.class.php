<?php
// Класс работы с группами свойств объектов CRM
class USAM_Property_Groups_Query 
{
	public $query_vars = [];
	private $results = [];
	private $total = 0;
	...
	public function prepare_query($query = array() ) 
	{
		...
		if ( 'all' == $qv['fields'] || 'id=>data' == $qv['fields'] || 'code=>data' == $qv['fields']) 		
			$this->query_fields = usam_get_table_db('property_groups').".*";
		else
		{
			...
			{							
				...
				$this->query_fields[] = usam_get_table_db('property_groups').".$field";
			}				
		...
		$this->query_from = "FROM ".usam_get_table_db('property_groups');
		...
		foreach ($ordersby as $_key => $_value ) 
		{
			...
			if ( 'group' == $_value ) 
			{
				...
				{
					$this->query_join .=  " INNER JOIN ".usam_get_table_db('property_groups')." ON (".usam_get_table_db('property_groups').".code = ".usam_get_table_db('property_groups').".group)";	
					if ( !empty($qv['type']) )
					{		
						$type = (array)$qv['type'];
		...
		{
			$orderby_array[] = usam_get_table_db('property_groups').".id $order";
		}
		...
		{
			$ids = implode( ',', $include );			
			$this->query_where .= " AND ".usam_get_table_db('property_groups').".id IN ($ids)";
		} 
		elseif ( !empty($qv['exclude']) ) 
		{
			$ids = implode( ',', wp_parse_id_list($qv['exclude']) );
			$this->query_where .= " AND ".usam_get_table_db('property_groups').".id NOT IN ($ids)";
		}			
		if ( isset($qv['code']) )
		{		
			$code = (array)$qv['code'];
			$this->query_where .= " AND ".usam_get_table_db('property_groups').".code IN ('".implode( "','", $code )."')";
		}			
		if ( !empty($qv['type']) )
		{		
			$type = (array)$qv['type'];
		...
		{		
			$parent_id = (array)$qv['parent_id'];
			$this->query_where .= " AND ".usam_get_table_db('property_groups').".parent_id IN ('".implode( "','", $parent_id )."')";
		}
		if ( isset($qv['system']) ) 
		{
			$system = $qv['system'] ? 1: 0;	
			$this->query_where .= " AND ".usam_get_table_db('property_groups').".system=$system";
		}	
		$this->meta_query = new USAM_Meta_Query();
		$this->meta_query->parse_query_vars($qv );	
		if ( !empty($this->meta_query->queries) ) 
		{
			$clauses = $this->meta_query->get_sql('group', usam_get_table_db('property_group_meta'), usam_get_table_db('property_groups'), 'id', $this );
	...
	public function query()
	{
		...
		{ 
			...
			{
				if ($qv['number'] == 1 )
					wp_cache_set($this->results->id, (array)$this->results, 'usam_property_group' );	
				else
				{					
					foreach ($this->results as $result ) 
					{
						...
						if ( !empty($result->code) )
							wp_cache_set($result->code, $result->id, 'usam_property_group_code' );						
					}
				}			
			}			
			if ( $qv['cache_meta'] )		
			{ 
				...
				usam_update_cache( $ids, array( usam_get_table_db('property_group_meta') => 'group_meta'), 'group_id' );
			}		
		}
	}
	...
	public function format_results( ) 
	{
		foreach ($this->results as $key => $result ) 
		{
			foreach (['id', 'parent_id', 'sort', 'system', 'type_payer_id'] as $column ) 
			{			
	...
	protected function get_search_sql($string, $cols, $wild = false ) 
	{
		...
		{
			if ( 'id' == $col || 'code' == $col ) 
				$searches[] = $wpdb->prepare( usam_get_table_db('property_groups').".$col = %s", $string );
			else 
				$searches[] = $searches[] = $wpdb->prepare( usam_get_table_db('property_groups').".$col LIKE %s", $like );
		}
	...
	protected function parse_orderby($orderby ) 
	{
		...
		{
			$_orderby = usam_get_table_db('property_groups').'.'.$orderby;
		} 	
		elseif ( in_array($orderby, array( 'sort', 'active' ) ) )
		{
			$_orderby = "(".usam_get_table_db('property_groups').'.'.$orderby."+0)";
		} 			
		elseif ( 'include' === $orderby && !empty($this->query_vars['include']) ) 
		{
			$include = wp_parse_id_list($this->query_vars['include']);
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".usam_get_table_db('property_groups').".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_property_groups( $query_args = [] )
{	
	$query_args['count_total'] = false;
	$query = new USAM_Property_Groups_Query($query_args );	
