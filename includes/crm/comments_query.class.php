...
class USAM_Comments_Query 
{
	...
	private $fields = [];
	private $db_fields = ['id' => 'd', 'object_id' => 'd', 'object_type' => 's', 'message' => 's', 'status' => 'd', 'user_id' => 'd', 'date_insert' => 's'];
	...
	public function prepare_query( $query = array() ) 
	{
		...
		do_action( 'usam_pre_get_comments', $this );
		...
		{
			if ( $field == 'all' )
				$this->fields[] = USAM_TABLE_COMMENTS.".*";
			elseif ( $field == 'count' )
				$this->fields[] = "COUNT(*) AS count";	
			elseif ( isset($this->db_fields[$field]) )
				$this->fields[] = USAM_TABLE_COMMENTS.".$field";			
		}
		if ( !count($this->fields) )
			$this->query_fields = USAM_TABLE_COMMENTS.".*";
		else
		...
		$this->query_from = "FROM ".USAM_TABLE_COMMENTS;
	...
	// Обрабатывать другие параметры даты
		...
		if ( $date_parameters ) 
		{
			$date_query = new USAM_Date_Query( [$date_parameters], USAM_TABLE_COMMENTS );
			$this->query_where .= $date_query->get_sql();
		}
		if ( !empty($qv['date_query']) ) 
		{
			$this->date_query = new USAM_Date_Query( $qv['date_query'], USAM_TABLE_COMMENTS );
		...
		if ( !empty($qv['user_id']) ) 
		{
			...
			if ( !empty($user_id) ) 
				$this->query_where .= " AND ".USAM_TABLE_COMMENTS.".user_id IN (".implode( ",", $user_id ).")";	
		}	
		if ( isset($qv['user_id__not_in']) ) 
		{
			$user_id__not_in = (array) $qv['user_id__not_in'];
			$this->query_where .= " AND ".USAM_TABLE_COMMENTS.".user_id NOT IN (".implode(",", $user_id__not_in ).")";
		}		
		// Группировать
		...
		{
			$orderby_array[] = USAM_TABLE_COMMENTS.".id $order";
		}
		...
		if ( $search ) 
		{
			...
			$search_columns = apply_filters( 'usam_comments_search_columns', $search_columns, $search, $this );
		...
		{
			$ids = implode( ',', $include );			
			$this->query_where .= " AND ".USAM_TABLE_COMMENTS.".id IN ($ids)";
		} 
		elseif ( !empty( $qv['exclude'] ) ) 
		{
			$ids = implode( ',', wp_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND ".USAM_TABLE_COMMENTS.".id NOT IN ($ids)";
		}	
		if ( isset($qv['object_id']) )
		{
			$object_id = implode( ',',  wp_parse_id_list((array)$qv['object_id']) );		
			$this->query_where .= " AND ".USAM_TABLE_COMMENTS.".object_id IN ($object_id)";
		}	
		if ( $qv['status'] !== 'all' )
		{
			$status = implode( ',',  wp_parse_id_list((array)$qv['status']) );		
			$this->query_where .= " AND ".USAM_TABLE_COMMENTS.".status IN ($status)";			
		}	
		if ( !empty( $qv['object_type'] ) ) 
		{		
			$object_type = implode( "','",  (array)$qv['object_type'] );		
			$this->query_where .= " AND ".USAM_TABLE_COMMENTS.".object_type IN ('$object_type')";
		}				
		do_action_ref_array( 'usam_pre_comments_query', array( &$this ) );
	}
	...
	public function handling_data_types( $data )
	{
		...
		if ( $this->query_vars['cache_results'] && isset($data->id) )
			wp_cache_set( $data->id, (array)$data, 'usam_comment' );	
		return $data;
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) 
	{
		...
		foreach ( $cols as $col ) 
		{
			if ( 'id' == $col ) 
			{
				$searches[] = $wpdb->prepare( USAM_TABLE_COMMENTS.".$col = %s", $string );
			} 			
			else 
			{
	...
	protected function parse_orderby( $orderby ) 
	{
		...
		if ( isset($this->db_fields[$orderby]) )
			$_orderby = USAM_TABLE_COMMENTS.'.'.$orderby;
		elseif ( $orderby == 'date' )
			$_orderby = USAM_TABLE_COMMENTS.'.date_insert';	
		elseif ( 'include' === $orderby && !empty( $this->query_vars['include'] ) ) {
			$include = wp_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( ".USAM_TABLE_COMMENTS.".id, $include_sql )";
		} 		
		return $_orderby;
	}
...
function usam_get_comments( $query = array() )
{	
	$query['count_total'] = false;	
	$comments = new USAM_Comments_Query( $query );	
